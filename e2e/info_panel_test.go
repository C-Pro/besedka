...
// TestE2EImageOverlayZoom verifies the fullscreen overlay is zoomable: a wheel
// zoom scales the image and marks the overlay zoomed, and navigating to another
// image resets the zoom back to 1x.
func TestE2EImageOverlayZoom(t *testing.T) {
	t.Parallel()
	server := startServer(t)
	defer server.Stop()
	...
	// Open the overlay on the latest image.
	err = alicePage.Locator("#last-image-box").Click()
	require.NoError(t, err)
	err = alicePage.Locator("#image-overlay.active").WaitFor(playwright.LocatorWaitForOptions{
		State: playwright.WaitForSelectorStateVisible,
	...
	// Initially unzoomed: no transform scale, no "zoomed" class.
	zoomedCount, err := alicePage.Locator("#image-overlay.zoomed").Count()
	require.NoError(t, err)
	require.Equal(t, 0, zoomedCount)

	// Wheel up over the image zooms in (same zoomTo path pinch uses).
	viewport := alicePage.ViewportSize()
	err = alicePage.Mouse().Move(float64(viewport.Width)/2, float64(viewport.Height)/2)
	require.NoError(t, err)
	err = alicePage.Mouse().Wheel(0, -400)
	require.NoError(t, err)

	// The overlay becomes zoomed and the image gains a scale > 1 transform.
	err = alicePage.Locator("#image-overlay.zoomed").WaitFor(playwright.LocatorWaitForOptions{
		State: playwright.WaitForSelectorStateVisible,
	})
	require.NoError(t, err)
	transform, err := alicePage.Locator("#overlay-image").Evaluate("el => el.style.transform", nil)
	require.NoError(t, err)
	require.Contains(t, transform.(string), "scale(")
	require.NotContains(t, transform.(string), "scale(1)")

	// Navigating to the previous image resets the zoom back to 1x.
	err = alicePage.Keyboard().Press("ArrowLeft")
	require.NoError(t, err)
	err = alicePage.Locator("#image-overlay.zoomed").WaitFor(playwright.LocatorWaitForOptions{
		State: playwright.WaitForSelectorStateHidden,
